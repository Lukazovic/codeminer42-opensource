import React, { ComponentProps } from 'react'
import css from '@styled-system/css'
import Box from './Box'
import { A, Flex } from '.'

const icons = {
  translate: {
    d: 'M253.227,300.267L253.227,300.267L199.04,246.72l0.64-0.64c37.12-41.387,63.573-88.96,79.147-139.307h62.507V64H192V21.333h-42.667V64H0v42.453h238.293c-14.4,41.173-36.907,80.213-67.627,114.347c-19.84-22.08-36.267-46.08-49.28-71.467H78.72c15.573,34.773,36.907,67.627,63.573,97.28l-108.48,107.2L64,384l106.667-106.667l66.347,66.347L253.227,300.267zM373.333,192h-42.667l-96,256h42.667l24-64h101.333l24,64h42.667L373.333,192z M317.333,341.333L352,248.853l34.667,92.48H317.333z',
    viewBox: '0 0 469.333 469.333',
    width: '10px',
    height: '17px',
    link: '',
  },
  facebook: {
    d: 'M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z',
    viewBox: '0 0 320 512',
    width: '10px',
    height: '17px',
    link: 'https://www.facebook.com/codeminer42',
  },
  linkedin: {
    d: 'M100.28 448H7.4V148.9h92.88zM53.79 108.1C24.09 108.1 0 83.5 0 53.8a53.79 53.79 0 01107.58 0c0 29.7-24.1 54.3-53.79 54.3zM447.9 448h-92.68V302.4c0-34.7-.7-79.2-48.29-79.2-48.29 0-55.69 37.7-55.69 76.7V448h-92.78V148.9h89.08v40.8h1.3c12.4-23.5 42.69-48.3 87.88-48.3 94 0 111.28 61.9 111.28 142.3V448z',
    viewBox: '0 0 448 512',
    width: '10px',
    height: '17px',
    link: 'https://www.linkedin.com/company/codeminer42',
  },
  twitter: {
    d: 'M459.37 151.716c.325 4.548.325 9.097.325 13.645 0 138.72-105.583 298.558-298.558 298.558-59.452 0-114.68-17.219-161.137-47.106 8.447.974 16.568 1.299 25.34 1.299 49.055 0 94.213-16.568 130.274-44.832-46.132-.975-84.792-31.188-98.112-72.772 6.498.974 12.995 1.624 19.818 1.624 9.421 0 18.843-1.3 27.614-3.573-48.081-9.747-84.143-51.98-84.143-102.985v-1.299c13.969 7.797 30.214 12.67 47.431 13.319-28.264-18.843-46.781-51.005-46.781-87.391 0-19.492 5.197-37.36 14.294-52.954 51.655 63.675 129.3 105.258 216.365 109.807-1.624-7.797-2.599-15.918-2.599-24.04 0-57.828 46.782-104.934 104.934-104.934 30.213 0 57.502 12.67 76.67 33.137 23.715-4.548 46.456-13.32 66.599-25.34-7.798 24.366-24.366 44.833-46.132 57.827 21.117-2.273 41.584-8.122 60.426-16.243-14.292 20.791-32.161 39.308-52.628 54.253z',
    viewBox: '0 0 512 512',
    width: '10px',
    height: '17px',
    link: 'https://twitter.com/codeminer42',
  },
}

type Props = ComponentProps<typeof Box> & {
  name: keyof typeof icons
}

const MediaIcon: React.FC<Props> = ({ name, color = 'black', ...props }) => (
  <Flex
    as='span'
    alignItems='center'
    justifyContent='center'
    backgroundColor='#f7f8fc'
    borderRadius='50%'
    size='30px'
    {...props}
  >
    <A href={icons[name].link} target='_blank' rel='noopener noreferrer'>
      <Box
        as='svg'
        width={icons[name].width}
        height={icons[name].height}
        viewBox={icons[name].viewBox}
        css={css({ fill: color })}
        {...props}
      >
        <path d={icons[name].d} />
      </Box>
    </A>
  </Flex>
)

export default MediaIcon
